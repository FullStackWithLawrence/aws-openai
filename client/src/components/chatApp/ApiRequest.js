    ...
    if (response.ok) {
      ...
      switch (status) {
        ...
        case 500:
          ...
          break;
        case 504:
          errMessage = response.statusText || 'Gateway timeout error. Note that AWS Lambda has a hard 29 second timeout. If OpenAI requests take longer, which is frequently the case with chatgpt-4 then you will receive this error. If the timeout persists then you might try using chatgpt-3.5 instead as it is more performant.';
          break;
      }
